...
rule([[ IfStatement <- IfPart (ElseIfPart)* ElsePart? ~EndKeyword ]], basic.concat )
...
rule([[ IfBody <- Skip (!ElseIfKeyword !ElseKeyword !EndKeyword LocalStatement Skip)* ]], basic.concat )
...
rule([[ SwitchStatement <- SwitchPart Skip CaseList ~EndKeyword ]], "{1}{2}{\n{3}\n}" )
...
-- since the syntax error part of local statement swallows everything, other possible branches have to be excluded
rule([[ CaseBody <- Skip (!CaseKeyword !DefaultKeyword !FallKeyword !EndKeyword LocalStatement Skip)* OptionalFallStatement Skip ]], basic.concat )
